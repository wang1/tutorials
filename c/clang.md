...
当然，除了`C`语言外，还有不少先进的高级语言。 推荐: `Go`，`Java`，`Python`，`Typescript`，`Dart`，`Rust`，`Scala`等等
...
Positive
: ✍ 注意：字符串以特殊的单个字符`\0`作为结束标志，且输出时使用数组名即可！
...
在实际编程中，对字符串的操作很常见，如两个字符串的比较、连接两个字符串、复制一个字符串等。所以`C`语言给我们提供了不少已经定义好的函数供直接调用。要使用这些函数，需要引入字符串相关的头文件：
...
Negative
: ✉ 参考答案:
1：`gets(yourPassword)` 或 `scanf("%s", yourPassword)`
...
请掌握[棋歌教学网实验四](../../c/c-experiment.html#4)第 7、8、10 题程序
...
Positive
: 🌏 恺撒密码（Caesar cipher）
或称恺撒加密、恺撒变换、变换加密，是一种最简单且最广为人知的加密技术。 它是一种替换加密的技术，明文中的所有字母都在字母表上向后（或向前）按照一个固定数目进行偏移后被替换成密文。 例如，当偏移量是`3`的时候，所有的字母`A`将被替换成`D`，`B`变成`E`，以此类推。
...
是的，我们说过，`C`语言程序是由函数构成的，有且只能有一个`main`函数，它是程序执行的入口和出口。
...
- 函数名前是函数的返回值类型如`int`、`float`等，如果没有返回值则写`void`
...
请理解本讲所列程序。
请练习并掌握 [棋歌教学网实验二中的 4 个程序](../../c/c-experiment.html#2)
...
Positive
: ✍ 请注意：指针是`C`语言的重点和难点，是瑞士军刀，也是双刃剑。我们当前只进行简要的学习。
...
看看你的计算机硬件参数，你会发现当前我们的`PC`一般都会有`4G/8G/16G`字节的容量（移动设备如手机一般要少一些）。

Positive
: ✍ 请注意区分内存和外存！一般你的外存如硬盘有`500G/1T`的容量，容量远高于内存，速度远低于内存。
...
比如现实生活中，你对我说需要`64`块钱，我可以直接给你`64`块，这叫直接获取；我也可以回答你：`64`块在我的上衣右下口袋中，告诉你地址/指针，这叫间接获取。
...
Positive
: ✍ 注意： 定义变量时，变量名前的`*`号表明其为一个指针变量，该变量只能存放地址！
...
Positive
: ✍ 注意：在指针`p`指向`x`的情况下即`p = &x`：`*p`与`x`等价，`p`与`&x`等价。至此，你应该明白为何我们以前一直强调`scanf`输入语句中，变量名前为何需要使用`&`符号的问题了。
...
如果你有比较多的数据，比如`100`个整数需要交给函数去处理，那么怎么办？

按照以前的做法，我们得为函数定义`100`个形参，这样是不科学的！
...
Positive
: ✍ 注意：`C`语言规定，数组名也是数组的首地址/指针，即与数组的首元素的地址等价！
...
Positive
: ✍ 注意：请一定留意上面程序中的三个函数都共用一个数组。这也是为何在`main`函数中定义的数组，在`inputData`函数中可以向该数组输入数据，也在`findMax`函数中可以得到的数据原因。

Negative
: ✎ 思考：`data`是数组名，也是数组的首地址，上面的程序我们进行了使用。那么我们可不可以传递`&data[0]`呢？
...
某些问题必须得用递归进行求解。如：[分形问题](https://zh.wikipedia.org/wiki/%E5%88%86%E5%BD%A2)、[汉诺塔问题](https://zh.wikipedia.org/wiki/%E6%B1%89%E8%AF%BA%E5%A1%94)等，有兴趣的同学可参阅其它资料。
...
运行该程序，结果将会打印`3`次`64`，请前往[可视化运行网站](http://www.pythontutor.com/c.html#mode=edit)探究程序的执行。
...
这些命令告诉`C`语言编译程序将对应的文件引入进来，本程序需要使用。如果没有，那么诸如`printf`之类的函数就不能使用。
